...
static uint8_t channelvelocities[] = {127, 127, 127, 127, 127, 127, 127, 127,
...
static const uint8_t controller_map[] = {0x00, 0x20, 0x01, 0x07, 0x0A,
...
static bool WritePressKey(uint8_t channel, uint8_t key, uint8_t velocity,
                          VFILE *midioutput)
{
...
static bool WriteReleaseKey(uint8_t channel, uint8_t key, VFILE *midioutput)
{
...
static bool WritePitchWheel(uint8_t channel, short wheel, VFILE *midioutput)
{
...
static bool WriteChangePatch(uint8_t channel, uint8_t patch, VFILE *midioutput)
{
...
static bool WriteChangeController_Valued(uint8_t channel, uint8_t control,
                                         uint8_t value, VFILE *midioutput)
{
...
static bool WriteChangeController_Valueless(uint8_t channel, uint8_t control,
                                            VFILE *midioutput)
{
...
static int GetMIDIChannel(int mus_channel, VFILE *midioutput)
{
	...
	if (mus_channel == MUS_PERCUSSION_CHAN) {
		...
		if (channel_map[mus_channel] == -1) {
			...
			// event. This fixes "The D_DDTBLU disease" described
			// here:
			// https://www.doomworld.com/vb/source-ports/66802-the
			WriteChangeController_Valueless(
			    channel_map[mus_channel], 0x7b, midioutput);
		}
...
static bool ReadMusHeader(VFILE *file, musheader *header)
{
	...
	result =
	    vfread(&header->id, sizeof(uint8_t), 4, file) == 4 &&
...
bool mus2mid(VFILE *musinput, VFILE *midioutput)
{
	...
	if (vfseek(musinput, (long) musfileheader.scorestart, SEEK_SET) != 0) {
		return true;
	}
	...
	while (!hitscoreend) {
		...
		while (!hitscoreend) {
			...
			channel =
			    GetMIDIChannel(eventdescriptor & 0x0F, midioutput);
			...
			case mus_presskey:
				...
				if (key & 0x80) {
					if (vfread(&channelvelocities[channel],
					           1, 1, musinput) != 1) {
						return true;
					}
			...
			case mus_systemevent:
				if (vfread(&controllernumber, 1, 1, musinput) !=
				    1) {
					return true;
				}
			...
			case mus_changecontroller:
				...
				if (vfread(&controllervalue, 1, 1, musinput) !=
				    1) {
					return true;
				}

				if (controllernumber == 0) {
					if (WriteChangePatch(channel,
					                     controllervalue,
					                     midioutput)) {
						return true;
					}
